      ...
      >
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            {/* Member list with transport/holes */}
            {group.members && group.members.length > 0 && (
              <div className="mb-3 space-y-0.5">
                {group.members.map(member => {
                  ...
                  return (
                    <div key={member.id} className="flex items-center gap-1.5 text-xs text-gray-600">
                      <span className="font-medium truncate">{member.name}</span>
                      <span className={`flex-shrink-0 ${transport === 'walking' ? 'text-green-600' : 'text-gray-400'}`}>
                        {transport === 'walking' ? '🚶' : '🛒'}
                      </span>
                      <span className={`flex-shrink-0 font-semibold ${holes === 9 ? 'text-amber-600' : 'text-gray-400'}`}>
                        {holes}
                      </span>
                    </div>
                  )
